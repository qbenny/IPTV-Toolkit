    ...
    template: `
        <div class="card-grid">
            <div class="card">
                ...
                <div class="form-actions mt-20"><button class="btn btn-primary w-full" @click="triggerSync" :disabled="syncStatus.running">{{ syncStatus.running ? '⏳ 同步中...' : '🔄 开始全量同步' }}</button></div>
            </div>
            <div class="card">
        ...
        <div class="card mt-20">
            ...
            <div class="status-list mt-10"><p class="status-desc">📥 数据来源：VIS API <code>api/search/filter.json</code><br>📊 同步类型：电视剧、电影、综艺、动漫、少儿<br>🔄 同步方式：全量覆盖（每日建议执行一次）<br>⚡ TVBox 接口基于本地 SQLite 数据库，支持 country / year 多条件过滤和评分排序</p></div>
        </div>
    `
};

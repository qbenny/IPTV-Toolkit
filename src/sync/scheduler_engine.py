...
- 直播同步：每天 live_sync_hour（默认 0）触发；已到钟点且进程已登录则顺带完成
  （零额外顶号）。若到钟点时处于休眠态（未登录）：近 24h 内有过登录则暂不唤醒，
  等待 3 点后首次真实登录顺带触发；仅当超过 24h 无任何登录（长期无人使用）才强制
  登录兜底，确保直播更新不长期缺失。严格按配置钟点执行，
...
- 可配置（B）：三个钟点存于 scheduler_config，调度器每次轮询动态读取。
...
LIVE_FALLBACK_IDLE_HOURS = 24   # 超过此时长（小时）无任何登录，才强制兜底登录
...
def _check_live(now, hour):
    ...
    if _sim.state.is_authenticated:
        ...
        return
    # 未登录（休眠态）：是否强制登录兜底取决于“距上次登录过了多久”
    # —— 近 24h 内有过登录：3 点后还会有真实请求触发登录并顺带同步，不唤醒机顶盒，
    #    仅标记 pending 等待；
    # —— 超过 24h 无任何登录（长期无人使用，不会有真实登录来触发）：才强制登录兜底，
    #    确保直播更新不长期缺失（受重试上限约束）。
    idle_hours = (now.timestamp() - _sim.state.last_login_time) / 3600.0
    if idle_hours < LIVE_FALLBACK_IDLE_HOURS:
        if not st["pending"]:
            st["pending"] = True
            logger.info("[Scheduler] 直播同步已到钟点(%02d:00)但机顶盒休眠；近 %.0f 小时内有登录记录，"
                        "暂不唤醒，等待 3 点后首次真实登录顺带触发。", hour, idle_hours)
        return
    # 超过阈值：强制登录兜底
